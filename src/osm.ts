...
function _osmtogeojson(data: JSON | Document | Element): FeatureCollection<GeometryObject, OsmObjectProperties> {
  return osmtogeojson(
    data,
...
const filterObjects = (xmlDom: Document, action: ADiffAction): OsmObjectDiff[] => {
  return Array.from(xmlDom.querySelectorAll(`action[type=${action}]`).entries()).map(([_, e]) => {
      const newElement = action === ADiffAction.Create ? e : e.lastElementChild;
      const oldElement = action === ADiffAction.Create ? null : e.firstElementChild;
      ...
      if (diff.old === null && diff.new === null) {
        console.warn(`${action} element missing old or new object!`, oldElement, newElement);
      }
      return diff;
    });
}

export const getAugmentedDiff = (sequenceId: string): Promise<AugmentedDiff> => {
  return fetchAdiffStub(sequenceId)
    .then((bodyText) => {
